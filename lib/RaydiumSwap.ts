...
class RaydiumSwap {
  ...
  constructor(RPC_URL: string, WALLET_PRIVATE_KEY: string) {
    ...
    this.wallet = new Wallet(Keypair.fromSecretKey(
      Uint8Array.from(JSON.parse(fs.readFileSync(WALLET_PRIVATE_KEY, 'utf-8'))),
      { skipValidation: true }));
  }

  async loadPoolKeys() {
    const liquidityJsonResp = await node_fetch('https://api.raydium.io/v2/sdk/liquidity/mainnet.json')
  ...
  async getSwapTransaction(
    ...
    fixedSide: 'in' | 'out' = 'in'
  ): Promise<{ transaction: Transaction, minAmountOut: TokenAmount | CurrencyAmount }> {
    ...
    return { transaction, minAmountOut };
